...
- **存放位置**：仓库根的**成片项目目录**里，跟该选题的 `deck/`、`*.mp4`、`transcript.md` **同级** —— 即 `<仓库根>/<选题目录>/script.md`。选题目录沿用既有惯例：抖音来源用「作者-标题前20字」（见 `小墨同学-…/`、`雨哥聊AI-…/`），自拟选题用描述性短名（见 `deck/`、`claude-code-deepseek/`）。**不落在 skill 目录、不落 `downloads/`**。
